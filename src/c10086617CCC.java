 ...
 *
 */
public class c10086617CCC extends Player {
	 ...
	 */
	public c10086617CCC(String pName) {super(pName);}
	...
	public KonaneMove makeMove(Konane game) {
		...
		if (possibleMoves.size() == 0) {
			...
			best = (int)maxVal(game,ply,ply); // the index of the best move
		}
	...
	private float maxVal(Konane game, int cutOff, int ply) {
		...
		if(cutOff == -1) // already reached to depth "ply"
			...
			float estimatesMax = -32768; // the maximum estimated value
			int index=0; 
			for(int i=0; i<possibleMoves.size(); i++) {
				...
				float tmp = minVal(newGame,cutOff,ply); //get the estimated value from the lower min level
	...
	private float minVal(Konane game,int cutOff,int ply){
		...
		if(cutOff == -1) // already reached to depth "ply"
			...
			for(int i=0; i<possibleMoves.size(); i++) {
				...
				float tmp = maxVal(newGame,cutOff,ply); //get the estimated value from lower max level
	...
	private float utility(Konane game) {
		float myMovablePieces = movablePieces(game,colour); // number of this player's movable pieces on current board of game
		float opMovablePieces = movablePieces(game,game.opponent(colour)); // number of opponent's movable pieces
	...
	private int movablePieces(Konane game, char player) {
		...
		for(i=1; i<num+1; i++)
			for(j=1; j<num+1; j++) {
				board[i][j] = temp[i-1][j-1]; //copy the original game board to the larger board
			}
		for(i=0; i<num+2; i++) {
			board[0][i] = board[i][0] = board[num+1][i] = board[i][num+1] = '@'; //set other position on board to '@'
		}
